...
async def uploader_worker(queue, ws):
    ...
    while True:
        ...
        buffer.append(item)
        if len(buffer) >= 250:
            print(f"🚀 [Group B] 250개 도달! 구글 시트 업로드 중...")
                ...
                for ad in await frame.locator("a").all():
                    ...
                    if any(k in href or k in img_src for k in ["addc.dc", "NetInsight", "nstatic", "toast"]):
                        ...
                        if key not in seen:
                            ...
                            print(f"✅ [B팀|{env}|{gallery[:4]}] {pos} 포착!")
...
async def main():
    print("⏳ [Group B] A팀이 구글 시트를 청소할 때까지 30초 대기합니다...")
    time.sleep(30) # A팀이 충돌 없이 청소할 시간을 넉넉히 줌
    print("🚀 [Group B] 20~37번 갤러리 수집 시작!")
